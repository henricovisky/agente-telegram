...
|-----------|------|------------|-------|
| Operação Local | Não-funcional | Crítica | O "core" deve rodar no servidor ubuntu 24.04 do usuário. |
| Interface Telegram | Funcional | Alta | Uso da biblioteca. |
| Persistência Local | Funcional | Alta | Armazenamento de conversas em SQLite. |
| Padronização de LLMs | Não-funcional | Alta | Troca dinâmica de provedores (Gemini 3.1 Flash e 2.5 Flash). |
