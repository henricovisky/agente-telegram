# Spec: PRD — Henricovisky Core

---

## 1. Resumo

O Henricovisky é um agente pessoal de Inteligência Artificial para operar 100% no servidor ubuntu 24.04 do usuário. Ele recebe comandos exclusivamente pelo Telegram, processa-os através de um pipeline que suporta múltiplos LLMs dinamicamente, e tem acesso persistente à memória em SQLite.
...
**Problema:**
Agentes hospedados na nuvem e serviços de terceiros requerem expor dados privados ou têm custos recorrentes altos, além da falta de total governança e controle sobre as próprias "skills" customizadas. O usuário não tem controle pleno de instâncias como o OpenClaw sem esbarrar na complexidade da nuvem ou lock-in.
...
**Por que agora:**
A ascensão de LLMs super eficientes e de api gratuita e de alta performance (gemini-3.1-flash-lite-preview, gemini-3-flash-preview, gemini-2.5-flash, gemini-2.5-flash-lite) somada com a facilidade da API do Telegram permitem rodar um agente pessoal sem os atritos operacionais de UI na web.
...
- [ ] G-02: Intercambiar "cérebros" (LLMs) usando padronização (Gemini).
...
**Usuário primário:** Henrique (proprietário), acessando via dispositivo móvel ou desktop via cliente Telegram, utilizando IDs em whitelist restrita.
...
O usuário envia um chat no Telegram, o Henricovisky roda local em background num terminal, chama LLMs, lê Skills em pastas locais, aciona ferramentas e responde no mesmo chat de forma orgânica.
